class sha256:
    def __init__(self, data: bytes = None, state: bytes = None, count: int = None):
        ...
        where 'state' is a 32-byte token and 'count' is the number of bytes consumed so far:
        ...
        if data is not None:
            if state is not None:
                raise TypeError(
                    'sha256() requires arguments \'data\' and \'state\' to not be used together'
                )
            if count is not None:
                raise TypeError(
                    'sha256() requires arguments \'data\' and \'count\' to not be used together'
                )
            ...
            if state is None or count is None:
                raise TypeError(
                    'sha256() requires arguments \'state\' and \'count\' to be used together'
                )
            if len(state) != 32:
                raise ValueError(
                    'sha256() requires argument \'state\' to be 32 bytes'
                )
            if count % 64 != 0:
                raise ValueError(
                    'sha256() requires argument \'count\' to be a multiple of 64'
                )
